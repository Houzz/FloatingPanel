...
//                  ", content safe area(bottom) =", safeAreaBottom)
    ...
    func updateHeight() {
        ...
        if layout is FloatingPanelIntrinsicLayout {
            ...
            if heightConstraints.isEmpty {
                let hc = surfaceView.heightAnchor.constraint(equalTo: vc.view.heightAnchor,
                                                             constant: -(safeAreaInsets.top + fullInset))
                hc.priority = .defaultHigh
                heightConstraints = [hc]
            } else {
                heightConstraints.first?.constant = -(safeAreaInsets.top + fullInset)
            }
        }
    ...
    func activateLayout(of state: FloatingPanelPosition) {
        ...
        switch state {
        case .full:
            fullConstraints.first?.constant = fullInset
            NSLayoutConstraint.activate(fullConstraints)
        case .half:
            halfConstraints.first?.constant = -halfInset
            NSLayoutConstraint.activate(halfConstraints)
        case .tip:
            tipConstraints.first?.constant = -tipInset
            NSLayoutConstraint.activate(tipConstraints)
        case .hidden:
            offConstraints.first?.constant = -hiddenInset
